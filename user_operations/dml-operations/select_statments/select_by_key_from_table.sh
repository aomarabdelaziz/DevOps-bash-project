...
column_val="$(awk "NR" Database/$dbName/$table | cut -d ";" -f$(($column_index)) | zenity --list --cancel-label="Back" --height="250" --width="300" --title="Table $table Column [$column]" --text="Select $column From $table"  --column="$column" 2>>.errorlog)"
